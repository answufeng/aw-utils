...
- **验证方式**：以仓库 **demo** 与集成侧手测为主（本仓库不维护库内单元测试）
...
- **本地建议**：`./gradlew :aw-utils:assembleRelease :aw-utils:ktlintCheck :aw-utils:lintRelease :demo:assembleRelease`
...
（发版命令见上文 **「工程品质与发版检查」**。）`demo` 为多 Tab Fragment；工具栏 **「演示清单」** 列出各 Tab，完整表见 [demo/DEMO_MATRIX.md](demo/DEMO_MATRIX.md)。弃用 API 见「弃用 API 迁移」。
