...
// 设备摘要（@AwExperimentalApi）
context.deviceSummary()     // "Xiaomi MI 11 | Android 14 (SDK 34)"
```
...
部分 API 标记了 `@AwExperimentalApi`，表示其行为在未来版本中可能变更。调用时需显式 Opt-In：

```kotlin
@OptIn(AwExperimentalApi::class)
val px = 100.dp                    // 无 Context 版本，使用 Resources.getSystem()
val friendly = timestamp.toFriendlyTime()  // 硬编码中文
```

当前标记为实验性的 API：
- `Number.dp` / `Number.dpF` / `Number.sp` / `Number.spF` — 使用 `Resources.getSystem()`，多窗口/折叠屏下可能不准确
- `Long.toFriendlyTime()` — 硬编码中文字符串，未来可能国际化
...
| 旧 API | 新 API | 说明 |
|--------|--------|------|
| `view.onClick { }` | `view.debounceClick { }` | 语义更清晰，避免与 setOnClickListener 冲突 |
| `view.postDelayed(300L) { }` | `view.postDelay(300L) { }` | 避免与 View.postDelayed 遮蔽 |
| `"text".ellipsize(5)` | `"text".truncate(5)` | 避免与 TextUtils.ellipsize 冲突 |
| `str.orDefault("x")` | `str ?: "x"` | 使用 Kotlin 惯用 Elvis 运算符 |
| `str.isNotNullOrBlank()` | `!str.isNullOrBlank()` | 使用 stdlib 原生写法 |
| `str.isNotNullOrEmpty()` | `!str.isNullOrEmpty()` | 使用 stdlib 原生写法 |
| `context.dp2px(16f)` | `16.dpToPx(context)` | Context 感知，多窗口安全 |
| `context.sp2px(14f)` | `14.spToPx(context)` | Context 感知，多窗口安全 |
| `context.px2dp(48)` | `48.pxToDp(context)` | 命名更一致 |
| `context.px2sp(42)` | `42.pxToSp(context)` | 命名更一致 |
| `deviceBrand`（顶层） | `context.deviceBrand` | Context 扩展，避免命名空间污染 |
| `deviceSummary()`（顶层） | `context.deviceSummary()` | Context 扩展 |
| `"aGVsbG8=".decodeBase64String()` | `"aGVsbG8=".decodeBase64ToString()` | 命名更符合 Kotlin 惯例 |
| `currentTimeMillis()` | `System.currentTimeMillis()` | 消除无意义包装 |
